"""
v9: EfficientNet-B4 (380px) - backbone升级验证
B4: 19M params, 380px native, ImageNet top1=83.4% (vs B2: 9M/260px/80.1%)
"""
...
LOG_FILE = os.path.join(LOG_DIR, f"training_v9_b4_{timestamp}.log")
...
# ============ Config ============
IMAGE_SIZE = 380  # B4 native
BATCH_SIZE = 24   # reduced for VRAM
GRAD_ACCUM = 2    # effective batch = 48
NUM_EPOCHS = 40
WARMUP_EPOCHS = 3
LR = 8e-4                  # slightly lower for larger model
FINE_LR = 3e-5
WD = 3e-4
LABEL_SMOOTH = 0.1
...
print(f"Backbone: EfficientNet-B4 | {IMAGE_SIZE}px | Batch={BATCH_SIZE}x{GRAD_ACCUM}")
print(f"Params: ~19M | Seed: {SEED}")

# ============ Augmentation (v8 tuned, scaled to 380px) ============
...
inf = model.classifier[1].in_features  # 1792
...
best_ckpt = os.path.join(OUTPUT_DIR, "best_model_v9_b4.pth")
t0 = time.time()
...
for ep in range(NUM_EPOCHS):
    ...
    model.train(); tl_ = 0.0; opt.zero_grad(set_to_none=True)
...
v9_tta = {CLASS_NAMES[i]: per_f1[i] for i in range(4)}
v9_tta["macro"] = f1m
print(f"{'':<12} {'v8(B2)':<10} {'v9(B4)':<10} {'Diff':<10}")
print("-"*42)
for cls in CLASS_NAMES:
    print(f"  {cls:<10} {v8_tta[cls]:.4f}     {v9_tta[cls]:.4f}     {v9_tta[cls]-v8_tta[cls]:+.4f}")
print(f"  {'macro':<10} {v8_tta['macro']:.4f}     {v9_tta['macro']:.4f}     {v9_tta['macro']-v8_tta['macro']:+.4f}")

print(f"\nCloudy<->Sunny:")
print(f"  v8(B2): c->s=29 s->c=32")
print(f"  v9(B4): c->s={cm[0][2]} s->c={cm[2][0]}")
