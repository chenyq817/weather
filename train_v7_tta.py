"""
v7: v5基底 + TTA推理
策略: 训练完全不改动(v5配置), 仅在测试时加TTA(5crop+flip集成)
"""
...
LOG_FILE = os.path.join(LOG_DIR, f"training_v7_tta_{timestamp}.log")
...
print(f"Training: Same as v5 | Testing: TTA (5crop + flip)")
...
# TTA: 5 crop + flip = 10 predictions averaged
TTA_SIZE = int(IMAGE_SIZE * 1.14)

def tta_predict(model, img, device):
    img_resized = transforms.Resize(TTA_SIZE)(img)
    crops = transforms.FiveCrop(IMAGE_SIZE)(img_resized)  # (tl, tr, bl, br, center)
...
_, vl_ds = random_split(fv, [ts_, vs_], generator=torch.Generator().manual_seed(42))
...
best_ckpt = os.path.join(OUTPUT_DIR, "best_model_v7_tta.pth")
t0 = time.time()
...
for ep in range(NUM_EPOCHS):
    ...
    if f1 > best_f1:
        ...
        torch.save({"e":ep,"model":model.state_dict(),"f1":f1,"acc":acc}, best_ckpt)
...
# ============ TTA Test ============
print("\n=== TTA Test (5crop + flip = 10 preds/img) ===")
...
sacc = accuracy_score(st, sp); sf1m = f1_score(st, sp, average="macro")
print(f"No-TTA Accuracy: {sacc:.4f} | F1: {sf1m:.4f}")
...
print(f"\n{'':<12} {'Base':<10} {'v5(GPU)':<10} {'v7(TTA)':<10}")
print("-"*42)
for i, cls in enumerate(CLASS_NAMES):
    print(f"  {cls:<10} {base[cls]:.4f}     {v5[cls]:.4f}    {per_f1[i]:.4f}")
print(f"  {'macro':<10} {base['macro']:.4f}     {v5['macro']:.4f}    {f1m:.4f}")
print(f"  TTA gain: {f1m - sf1m:+.4f}")

print(f"\nCloudy<->Sunny:")
print(f"  Base: c->s=44 s->c=32")
print(f"  v5:   c->s=41 s->c=25")
print(f"  v7:   c->s={cm[0][2]} s->c={cm[2][0]}")
