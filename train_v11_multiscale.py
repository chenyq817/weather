"""
v11: ConvNeXt-Tiny + 多尺度池化头
- 1x1 pool: 全局亮度/色调 (768d)
- 2x2 pool: 天空/地面比例, 云层分布 (3072d)
- 4x4 pool: 局部纹理, 雨痕位置 (channel-reduced 2048d)
- Concat → 5888d → 256 → 4
"""
...
LOG_FILE = os.path.join(LOG_DIR, f"training_v11_multiscale_{timestamp}.log")
...
GRAD_CLIP = 1.0; EARLY_PATIENCE = 10; SEED = 42

CLASS_NAMES = ["cloudy", "rain", "sunny", "snow"]
NUM_CLASSES = len(CLASS_NAMES)
DEVICE = torch.device("cuda"); USE_AMP = True

print(f"Device: {DEVICE} | ConvNeXt-Tiny + MultiScale Head | {IMAGE_SIZE}px | Batch={BATCH_SIZE}")

# ============ Multi-Scale Head ============
class MultiScaleHead(nn.Module):
    def __init__(self, in_ch=768, num_classes=4, red=128):
        super().__init__()
        self.reduce = nn.Conv2d(in_ch, red, 1)  # channel reduction for 4x4
        total_dim = in_ch + in_ch * 4 + red * 16  # 768 + 3072 + 2048 = 5888
    ...
    def forward(self, x):
        s1 = F.adaptive_avg_pool2d(x, 1).flatten(1)           # global
        s2 = F.adaptive_avg_pool2d(x, 2).flatten(1)           # 4-quadrant
        s4 = F.adaptive_avg_pool2d(self.reduce(x), 4).flatten(1)  # 16-patch
        return self.fusion(torch.cat([s1, s2, s4], dim=1))
...
# ============ Model ============
print("\nBuilding ConvNeXt-Tiny + MultiScale Head...")
model = models.convnext_tiny(weights=models.ConvNeXt_Tiny_Weights.IMAGENET1K_V1)
model.avgpool = nn.Identity()  # disable default pooling, pass feature map to head
model.classifier = MultiScaleHead(in_ch=768, num_classes=NUM_CLASSES, red=128)
...
best_ckpt = os.path.join(OUTPUT_DIR, "best_model_v11_multiscale.pth")
t0 = time.time()
...
# ============ Compare ============
print(f"\n{'='*60}\n=== vs v10 (CT plain head) ===")
v10_tta = {"cloudy":0.7896, "rain":0.9157, "sunny":0.8465, "snow":0.9646, "macro":0.8791}
v11_tta = {CLASS_NAMES[i]: per_f1[i] for i in range(4)}; v11_tta["macro"] = f1m
print(f"{'':<12} {'v10(plain)':<12} {'v11(ms)':<12} {'Diff':<10}")
print("-"*46)
for cls in CLASS_NAMES:
    print(f"  {cls:<10} {v10_tta[cls]:.4f}       {v11_tta[cls]:.4f}       {v11_tta[cls]-v10_tta[cls]:+.4f}")
print(f"  {'macro':<10} {v10_tta['macro']:.4f}       {v11_tta['macro']:.4f}       {v11_tta['macro']-v10_tta['macro']:+.4f}")
print(f"\nCloudy<->Sunny: v10(CT): c->s=28 s->c=22")
print(f"                 v11(MS): c->s={cm[0][2]} s->c={cm[2][0]}")
print(f"\nModel: {best_ckpt}\nLog: {LOG_FILE}")
