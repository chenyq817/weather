"""Quick test evaluation for best_model_v3.pth"""
...
CKPT_PATH = os.path.join(BASE_DIR, "output", "best_model_v3.pth")
IMAGE_SIZE = 260
...
print(f"Best epoch info: ", end="")
ckpt = torch.load(CKPT_PATH, map_location=DEVICE, weights_only=False)
print(f"Epoch {ckpt['epoch']+1}, val_f1={ckpt['f1']:.4f}, val_acc={ckpt['acc']:.4f}")
...
print(f"\n=== v3 测试结果 (Epoch {ckpt['epoch']+1} 最佳模型, {IMAGE_SIZE}px, B2) ===")
print(f"Accuracy: {acc:.4f} | F1 macro: {f1m:.4f} | 推理: {tt:.1f}s ({tt/len(test_dataset)*1000:.1f}ms/img)")
print(f"\n分类报告:")
...
print(f"{'类别':<12} {'Baseline(B0)':<14} {'v1(CLAHE+CBAM)':<16} {'v3(B2+260px)':<16}")
print("-"*58)
for i, cls in enumerate(CLASS_NAMES):
    print(f"  {cls:<10} {base[cls]:.4f}         {v1[cls]:.4f}           {per_f1[i]:.4f}")
print(f"  {'macro avg':<10} {base['macro']:.4f}         {v1['macro']:.4f}           {f1m:.4f}")
print(f"\n阴天↔晴天混淆:")
