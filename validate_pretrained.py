"""
方案A: 天气预训练权重验证 - Fold1 backbone -> seed=123
1. 加载best_model_v8_fold0.pth的backbone权重(跳过分类头)
2. 随机初始化新分类头
3. 在seed=123新划分上训练(更低学习率)
4. 对比: ImageNet初始化(82.68%) vs 天气预训练初始化(??%)
"""
...
LOG_FILE = os.path.join(LOG_DIR, f"validate_pretrained_{timestamp}.log")
...
WARMUP_EPOCHS = 3
LR, FINE_LR, WD = 5e-4, 2e-5, 3e-4  # halved for weather-pretrained init
...
print(f"New split seed: {NEW_SEED}")
print(f"Init: best_model_v8_fold0.pth (weather pretrained backbone)")
print(f"LR: {LR} head / {FINE_LR} backbone (halved vs ImageNet init)")
...
# ============ Model ============
def build_model():
    model = models.efficientnet_b2(weights=None)  # no ImageNet
    inf = model.classifier[1].in_features
    # temp: match checkpoint classifier structure
    model.classifier = nn.Sequential(
        nn.Dropout(0.4), nn.Linear(inf, 256), nn.BatchNorm1d(256),
        nn.ReLU(), nn.Dropout(0.25), nn.Linear(256, NUM_CLASSES),
    )
    # Load weather-pretrained weights (structure now matches)
    pretrain_ckpt = os.path.join(OUTPUT_DIR, "best_model_v8_fold0.pth")
    pretrain = torch.load(pretrain_ckpt, map_location="cpu", weights_only=False)
    model.load_state_dict(pretrain["model"])
    # Re-randomize classifier head (backbone stays weather-pretrained)
    model.classifier = nn.Sequential(
        nn.Dropout(0.4), nn.Linear(inf, 256), nn.BatchNorm1d(256),
        nn.ReLU(), nn.Dropout(0.25), nn.Linear(256, NUM_CLASSES),
    )
...
TMP_DIR = os.path.join(OUTPUT_DIR, "_tmp_pretrained")
...
ckpt_path = os.path.join(OUTPUT_DIR, f"validate_pretrained_seed{NEW_SEED}.pth")
t0 = time.time()
...
# ============ Step 4: Compare ============
print(f"\n{'='*60}")
print("=== Step 4: Init Method Comparison (same seed=123 split) ===")
print(f"{'':<25} {'ImageNet Init':<16} {'Weather Pretrain':<16} {'Diff':<10}")
print("-"*67)

imagenet_f1 = 0.8268  # from validate_split123 run
imagenet_no_tta = 0.8097
imagenet_per = [0.7056, 0.8148, 0.8244, 0.9626]

print(f"  {'Test F1 (TTA)':<25} {imagenet_f1:.4f}             {f1_tta:.4f}             {f1_tta - imagenet_f1:+.4f}")
print(f"  {'Test F1 (no-TTA)':<25} {imagenet_no_tta:.4f}             {f1_no:.4f}             {f1_no - imagenet_no_tta:+.4f}")

print(f"\nPer-class (TTA):")
print(f"  {'':<12} {'ImageNet':<14} {'WeatherPretrain':<14} {'Gain':<10}")
for i, cls in enumerate(CLASS_NAMES):
    print(f"  {cls:<10} {imagenet_per[i]:.4f}           {per_tta[i]:.4f}           {per_tta[i] - imagenet_per[i]:+.4f}")
...
diff = f1_tta - imagenet_f1
if diff > 0.005:
    print(f"Weather pretrain beats ImageNet by {diff:+.4f}!")
    print("Weather-domain pretraining is effective.")
elif abs(diff) <= 0.005:
    print(f"Tie ({diff:+.4f}). Weather pretrain ~= ImageNet on this split.")
else:
    print(f"ImageNet won by {-diff:.4f}. Weather pretrain not better on this split.")
