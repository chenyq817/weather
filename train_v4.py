"""
v4训练: v3配置 + 类别加权Loss
策略: EfficientNet-B2 + 260px + RandomEqualize + 40epoch + EarlyStopping
加权CE Loss: 基于v3混淆矩阵, 阴天(1.4)和晴天(1.3)更高权重
目标: 修复sunny→cloudy回归问题
"""
...
# ============ 超参数 ============
IMAGE_SIZE = 260           # 224→260, 更多天空纹理细节
BATCH_SIZE = 12            # B2更大, 减小batch适配CPU内存
NUM_EPOCHS = 40            # 30→40, 更多轮次收敛
WARMUP_EPOCHS = 3
LEARNING_RATE = 1e-3
FINE_TUNE_LR = 5e-5
WEIGHT_DECAY = 3e-4
LABEL_SMOOTHING = 0.1
# 类别权重: 基于v3混淆矩阵错误率反比 (cloudy最难 sunny次之)
CLASS_WEIGHTS = torch.tensor([1.40, 0.95, 1.30, 0.90])  # cloudy, rain, sunny, snow
MIXUP_ALPHA = 0.3
CUTMIX_ALPHA = 0.2
MIXUP_PROB = 0.7
GRAD_CLIP = 1.0
...
print(f"Device: {DEVICE} | Size: {IMAGE_SIZE}px | Batch: {BATCH_SIZE}")
print(f"Model: EfficientNet-B2 | Epochs: {NUM_EPOCHS} | EarlyStop: {EARLY_STOP_PATIENCE}")
print(f"MixUp={MIXUP_ALPHA} CutMix={CUTMIX_ALPHA} LabelSmooth={LABEL_SMOOTHING}")
print(f"ClassWeights: cloudy={CLASS_WEIGHTS[0]:.2f} rain={CLASS_WEIGHTS[1]:.2f} sunny={CLASS_WEIGHTS[2]:.2f} snow={CLASS_WEIGHTS[3]:.2f}")
...
tl = DataLoader(train_dataset, BATCH_SIZE, shuffle=True, num_workers=0, drop_last=True)
vl = DataLoader(val_dataset, BATCH_SIZE, shuffle=False, num_workers=0)
tl_test = DataLoader(test_dataset, BATCH_SIZE, shuffle=False, num_workers=0)
...
print(f"{'类别':<12} {'Base(B0)':<10} {'v3(B2+260)':<12} {'v4(+Weight)':<12}")
print("-"*46)
